...
const Modal = ({isOpen, onClose, title, subtitle, children, size = 'md', hideClose = false}) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            ...
            />
            <section
                className={`relative flex max-h-[calc(100vh-32px)] w-full flex-col overflow-hidden rounded-3xl border border-white/70 bg-white shadow-modal animate-scale-in ${sizeClasses[size] || sizeClasses.md}`}>
                {(title || !hideClose) && (
                    <header className="flex items-start justify-between gap-4 border-b border-line px-5 py-4 sm:px-6">
                        ...
                        {!hideClose && (
                            <button type="button" onClick={onClose} className="icon-button-sm -mr-1"
                                    aria-label="Close modal">
                                <Icon name="x" size={18}/>
                            </button>
                        )}
