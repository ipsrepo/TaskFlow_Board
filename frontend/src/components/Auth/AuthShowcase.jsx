...
const DashboardPreview = () => (
  <div className="relative mx-auto mt-10 w-full max-w-[530px] animate-slide-up sm:mt-12">
    ...
    <div className="relative rounded-2xl border border-white/40 bg-white p-3.5 shadow-[0_24px_48px_rgba(9,30,66,0.24)] sm:p-4">
      ...
      <div className="mt-3 grid grid-cols-[1.06fr_0.94fr] gap-3">
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-slate-100 p-2.5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="rounded-md bg-primary-50 px-1.5 py-0.5 text-[8px] font-semibold text-primary-700">This week</span>
            </div>
        ...
        <div className="rounded-xl border border-slate-100 p-2.5">
          ...
          <div className="mt-3 flex items-center justify-center">
            <div className="relative flex h-[76px] w-[76px] items-center justify-center rounded-full border-[9px] border-primary-100 border-t-primary border-r-primary">
        ...
        <div className="space-y-2">
          {['Design review is ready for approval', 'Landing page tasks moved to QA', 'New research items added to the sprint'].map((item, index) => (
            <div key={item} className="flex items-center gap-2">
              <span className={`h-1.5 w-1.5 rounded-full ${index === 0 ? 'bg-success' : index === 1 ? 'bg-primary' : 'bg-warning'}`} />
    ...
    <div className="absolute -right-8 bottom-8 hidden w-40 rounded-xl border border-white/50 bg-white p-3 shadow-[0_14px_28px_rgba(9,30,66,0.18)] sm:block">
      <div className="flex items-center justify-between">
      ...
      <div className="mt-2 flex h-10 items-end gap-1">
        {[46, 65, 34, 82, 58, 74].map((height, index) => (
          <span key={`${height}-${index}`} className="flex-1 rounded-t-sm bg-primary-200 last:bg-primary" style={{ height: `${height}%` }} />
        ))}
      </div>
    </div>
  </div>
...
const AuthShowcase = () => (
    ...
    <div className="pointer-events-none absolute inset-0 opacity-80 [background-image:radial-gradient(circle_at_0%_0%,rgba(255,255,255,0.16),transparent_31%),radial-gradient(circle_at_100%_100%,rgba(255,255,255,0.13),transparent_34%),linear-gradient(rgba(255,255,255,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.05)_1px,transparent_1px)] [background-size:auto,auto,96px_96px,96px_96px]" />
    <div className="pointer-events-none absolute -right-20 -top-20 h-80 w-80 rounded-full border-[34px] border-white/10" />
    <div className="pointer-events-none absolute -bottom-24 -left-24 h-72 w-72 rounded-full border-[40px] border-white/10" />

    <div className="relative">
      <span className="inline-flex rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs font-semibold tracking-wide text-white/85 backdrop-blur-sm">
        Work, organized
      </span>
    ...
    <div className="relative mt-8 flex items-center gap-3 text-xs text-white/75">
      <span className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-white/20 bg-white/10 text-sm font-bold text-white">✓</span>
