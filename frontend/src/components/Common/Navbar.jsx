    ...
    return (
        <header className="sticky top-0 z-30 h-16 border-b border-line bg-white/95 shadow-topbar backdrop-blur">
            <div className="flex h-full items-center gap-3 px-4 lg:px-5">
                <button type="button" onClick={onMenuToggle} className="icon-button lg:hidden"
                ...
                <Link to="/dashboard" className="flex shrink-0 items-center gap-2.5" aria-label="TaskBoard home">
                    <span
                        className="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-primary text-sm font-bold text-white shadow-sm">T</span>
                    <span
                        className="hidden text-base font-bold tracking-[-0.03em] text-secondary sm:block">TaskBoard</span>
                </Link>

                <form onSubmit={submitSearch} className="mx-auto hidden w-full max-w-md md:block">
                    <label className="relative block">
                        <span
                            className="pointer-events-none absolute inset-y-0 left-3 flex items-center text-slate-400"><Icon
                            name="search" size={17}/></span>
                        <input
                            type="search"
                            value={searchValue}
                ...
                <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
                    ...
                    <Dropdown
                        trigger={
                            <button type="button"
                                    className="rounded-full p-0.5 transition-shadow hover:ring-4 hover:ring-primary-50"
