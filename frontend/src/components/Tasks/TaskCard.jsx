    ...
    >
      <div className="mb-3 flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          {task.description && <p className="mt-1.5 line-clamp-2 text-xs leading-5 text-muted">{task.description}</p>}
        </div>
        <button type="button" onClick={(event) => event.stopPropagation()} className="icon-button-sm -mr-1 -mt-1 h-7 w-7 opacity-0 transition-opacity group-hover:opacity-100" aria-label={`More options for ${task.title}`}><Icon name="more" size={17} /></button>
      </div>

      <div className="flex flex-wrap items-center gap-2">
        <Badge variant={priority.variant} size="sm"><Icon name="flag" size={12} className={priority.iconClass} />{priority.label}</Badge>
        {task.isOverdue && <Badge variant="danger" size="sm" dot>Overdue</Badge>}
        {currentBoard && <span className="h-2 w-2 rounded-full" style={{ backgroundColor: currentBoard.color }} aria-label={currentBoard.name} />}
      </div>
      ...
      <div className="mt-3 flex min-h-6 items-center justify-between gap-3 text-xs text-muted">
        <div className={`flex min-w-0 items-center gap-1.5 ${task.isOverdue ? 'font-medium text-danger' : ''}`}>
          {task.deadline ? <><Icon name="calendar" size={14} /> <span>{task.isOverdue ? `Overdue ${formatDistanceToNow(new Date(task.deadline))}` : format(new Date(task.deadline), 'MMM d')}</span></> : <span />}
        </div>
        <div className="flex items-center gap-2">
          {commentsCount > 0 && <span className="flex items-center gap-1"><Icon name="message" size={14} />{commentsCount}</span>}
