...
const SortableBoardRow = ({ board, isEditing, editValues, onEdit, onCancel, onChange, onSave, onDelete, isSaving }) => {
  const { attributes, listeners, setNodeRef, setActivatorNodeRef, transform, transition, isDragging } = useSortable({ id: boardId(board) });
  const style = { transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.55 : 1 };

  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-3 rounded-xl border border-line bg-white px-3 py-2.5 shadow-sm">
      <button ref={setActivatorNodeRef} type="button" className="icon-button-sm cursor-grab active:cursor-grabbing" aria-label={`Reorder ${board.name}`} {...attributes} {...listeners}>
        <Icon name="handle" size={17} />
      </button>

      {isEditing ? (
        <>
          <input type="color" value={editValues.color || board.color} onChange={(event) => onChange('color', event.target.value)} className="h-8 w-8 cursor-pointer rounded-lg border border-line bg-white p-0.5" aria-label="Board color" />
          <input value={editValues.name} onChange={(event) => onChange('name', event.target.value)} disabled={board.isDefault} className="input-base h-9 flex-1 py-1.5" aria-label="Board name" />
          <Button loading={isSaving} onClick={() => onSave(board)}>Save</Button>
          <Button size="sm" variant="ghost" onClick={onCancel} aria-label="Cancel editing"><Icon name="x" size={16} /></Button>
        </>
      ) : (
        <>
          ...
          {board.isDefault && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-500">Default</span>}
          <button type="button" onClick={() => onEdit(board)} className="rounded-lg px-2 py-1 text-xs font-semibold text-primary-700 transition-colors hover:bg-primary-50">Edit</button>
          {!board.isDefault && <button type="button" onClick={() => onDelete(board)} className="rounded-lg px-2 py-1 text-xs font-semibold text-danger transition-colors hover:bg-red-50">Delete</button>}
        </>
      )}
    </div>
  );
...
const StatusBoardManager = ({ project, onClose, onUpdate }) => {
  ...
  const addBoard = async () => {
    ...
    const temporaryBoard = { _id: `temporary-${Date.now()}`, name, color: newBoard.color, order: boards.length, isDefault: false };
  ...
  const saveBoard = async (board) => {
    ...
    const previousBoards = boards;
    const optimisticBoards = boards.map((item) => item.name === board.name ? { ...item, name: board.isDefault ? item.name : name, color: editValues.color || item.color } : item);
    synchronizeBoards(optimisticBoards);
    setEditingBoardName(null);
    setError('');
    setIsSaving(true);

    const response = await updateStatusBoard(project._id, board.name, { name: board.isDefault ? board.name : name, color: editValues.color });
    if (response?.success) {
      synchronizeBoards(getBoards(response));
    } else {
      synchronizeBoards(previousBoards);
  ...
  return (
    <Modal isOpen onClose={onClose} title="Manage status boards" subtitle="Add, rename, color, or reorder the workflow stages." size="lg">
      <div className="space-y-6 p-5 sm:p-6">
        ...
        <section className="border-t border-line pt-5">
          ...
          <p className="mt-1 text-sm text-muted">New boards appear in the board immediately after you add them.</p>
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
            <Input label="Board name" value={newBoard.name} onChange={(event) => setNewBoard((current) => ({ ...current, name: event.target.value }))} placeholder="For example, QA review" className="flex-1" />
            <label className="block">
              <span className="field-label">Color</span>
              <input type="color" value={newBoard.color} onChange={(event) => setNewBoard((current) => ({ ...current, color: event.target.value }))} className="h-10 w-full cursor-pointer rounded-xl border border-line bg-white p-1 sm:w-14" aria-label="New board color" />
            </label>
            <Button loading={isSaving} onClick={addBoard} icon={<Icon name="plus" size={17} />}>Add board</Button>
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            {colorPresets.map((color) => (
              <button key={color} type="button" onClick={() => setNewBoard((current) => ({ ...current, color }))} className={`h-6 w-6 rounded-full ring-offset-2 transition-transform hover:scale-110 ${newBoard.color === color ? 'ring-2 ring-secondary' : ''}`} style={{ backgroundColor: color }} aria-label={`Use color ${color}`} />
            ))}
          </div>
        </section>

        <div className="flex justify-end border-t border-line pt-5"><Button variant="outline" onClick={onClose}>Done</Button></div>
      </div>
    </Modal>
  );
};
